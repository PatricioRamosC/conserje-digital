...
use Throwable;
use App\Models\Generales\ServicioGeneral;
...
class ServicioGeneralController extends Controller
{
    public function index($condominio_id) {
        try {
            $servicios = ServicioGeneral::where('condominio_id', $condominio_id)
                ->get();
            return $this->responseOK($servicios);
    ...
    public function store(Request $request)
    {
        ...
        try {
            $delivery = ServicioGeneral::create($request->all());
